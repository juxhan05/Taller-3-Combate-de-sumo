...
import servidor.modelo.Kimarite;
 ...
 *
 * <p>
 * Esta es la clase más crítica del sistema en términos de concurrencia. Actúa
 * como <b>monitor Java</b>: sincroniza los dos hilos (luchadores) para que cada
 * uno espere el turno del otro.</p>
 *
...
public class Dohyo {
    ...
    /**
     * Probabilidad de sacar al oponente con un kimarite. 15% → la mayoría de
     * veces el rival se mantiene, pero eventualmente es sacado (combate
     * dinámico).
     */
    private static final double PROBABILIDAD_SAQUE = 0.05;

    /**
     * Espera máxima en ms para que el oponente lance su kimarite. El enunciado
     * fija este tope en 500 ms.
     */
    private static final int MAX_ESPERA_MS = 800;
    ...
    /**
     * Índice del luchador con turno de ataque. 0 = luchador1 ataca; 1 =
     * luchador2 ataca.
     */
    ...
    /**
     * Generador de números aleatorios para sorteo de turno y evaluación de
     * saque.
     */
     ...
     *
     * <p>
     * El primer luchador en llegar hace {@code wait()} hasta que el segundo
     * llegue y lo despierte con {@code notifyAll()}. En ese momento se asignan
     * rivales mutuamente y se sortea el turno inicial.</p>
     *
     * <p>
     * El método es {@code synchronized} para que la operación de contar
    ...
    // ── Ejecución de turno ────────────────────────────────────────────────────
    /**
     * Ejecuta el turno de un luchador: espera su turno, aplica kimarite, evalúa
     * el saque y cede el turno al rival.
     *
     * <h3>Detalle del mecanismo de espera:</h3>
     * <p>
     * El hilo llama {@code wait(500)} mientras no sea su turno.
     * {@code wait(500)} es diferente a {@code wait()}: libera el monitor y
     * espera como máximo 500ms antes de reevaluar la condición, cumpliendo el
     * requerimiento del enunciado.</p>
     *
     * <h3>Evaluación de saque:</h3>
     * <p>
     * Se genera un {@code double} entre 0.0 y 1.0. Solo si es menor a
     * {@link #PROBABILIDAD_SAQUE} (5%) el rival es sacado. Esto hace que la
     * mayoría de turnos el rival se mantenga en pie, pero eventualmente sea
     * eliminado.</p>
     *
     ...
     * @throws InterruptedException Si el hilo es interrumpido durante la
     * espera.
     */
